...
int utn_isValidDigitoVerificador (char* cadena )
{
    ...
    int retorno = TRUE;
    int i;
    int mult [LONG_CUIT-1] = {2,3,4,5,6,7,2,3,4,5};//revisar
    ...
    //printf(" pos i %d \n", total);
    verificador = LONG_CUIT - (total % LONG_CUIT);// total % 11
    //printf("digito %d \n",verificador);
    if(verificador != cadena[LONG_CUIT])////revisar
    {
        retorno = FALSE;
    }
...
int utn_getChar(char* msg, char* msgError, int min, int max, int reintentos, char* resultado)
{
    ...
    if(msg!=NULL && msgError!=NULL && min<=max && reintentos>=0 && resultado!=NULL)
    {
        do
        {
            ...
            {
                if(utn_isValidChar(bufferChar, min, max))
                {
                    printf("OK");
